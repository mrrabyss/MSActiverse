...
fn unzip<P: AsRef<Path>>(archive_path: P, output_dir: P) -> Result<(), Box<dyn std::error::Error>> {
    let file = fs::File::open(&archive_path)?;
    let mut archive = zip::ZipArchive::new(file)?;

    for i in 0..archive.len() {
        let mut entry = archive.by_index(i)?;
        let out_path = Path::new(output_dir.as_ref()).join(entry.mangled_name());
...
pub fn install_spcc(key: &str) -> std::process::Output {
    println!("[*] Downloading spcc.dll");
    download("https://github.com/asdcorp/ohook/releases/download/0.5/ohook_0.5.zip")
        .expect("[!] Failed to download spcc.dll");
    unzip("ohook_0.5.zip", "C:\\ohook").expect("[!] Failed to extract the archive!");
    println!("[*] Doing some black magic...");
    let _ = Command::new("cmd")
    .args(["/C", r#"mklink "%ProgramFiles%\Microsoft Office\root\vfs\System\sppcs.dll" "%windir%\System32\sppc.dll""#]) 
    .output()
    .expect("[!] Failed to execute command");
    let _ = Command::new("cmd")
    .args(["/C", r#"cd /d C:\ohook && copy /y sppc64.dll "%ProgramFiles%\Microsoft Office\root\vfs\System\sppc.dll"#]) 
    .output()
    .expect("[!] Failed to execute command");
    let _ = Command::new("cmd")
    .args(["/C", r#"reg add HKCU\Software\Microsoft\Office\16.0\Common\Licensing\Resiliency /v "TimeOfLastHeartbeatFailure" /t REG_SZ /d "2040-01-01T00:00:00Z" /f"#]) 
    .output()
    .expect("[!] Failed to execute command");
    // activate
    println!("[*] Activating...");
    let output = Command::new("cmd")
        .args(["/C", &format!("slmgr.vbs /ipk {}", key)])
        .output()
        .expect("[!] Failed to execute command");
    return output;
}
pub static OFFICE_PRODUCT_KEYS: LazyLock<HashMap<&'static str, &'static str>> =
    LazyLock::new(|| {
        HashMap::from([
            // Office 365
            ...
            (
                "HomeStudentPlusARM2019Retail",
                "NM8WT-CFHB2-QBGXK-J8W6J-GVK8F",
            ),
            ...
            (
                ...
                "JBDKF-6NCD6-49K3G-2TV79-BKP73",
            ),
            (
                "SkypeforBusiness2019Volume",
                "9MNQ7-YPQ3B-6WJXM-G83T3-CBBDK",
            ),
            (
                "SkypeforBusinessEntry2019Retail",
                "N9722-BV9H6-WTJTT-FPB93-978MK",
            ),
            ...
            (
                ...
                "DVBXN-HFT43-CVPRQ-J89TF-VMMHG",
            ),
            (
                "SkypeforBusiness2021Volume",
                "R3FCY-NHGC7-CBPVP-8Q934-YTGXG",
            ),
            ...
            (
                "SkypeforBusiness2024Volume",
                "XKRBW-KN2FF-G8CKY-HXVG6-FVY2V",
            ),
...
pub fn find_office_product(product: &str) -> Option<&'static str> {
    OFFICE_PRODUCT_KEYS.get(product).copied()
}
